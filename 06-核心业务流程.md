...
Pod 的 Phase 是由其内部每个容器的 State 决定的。容器虽然只有三种真实状态（Waiting, Running, Terminated），但 K8s 会附带一个 `Reason`（原因）字段。**我们在排错时经常看到的那些“状态”，其实都是 Reason！**

* **Waiting (等待)**：容器没有在运行。常见 Reason 包括：
  * `ContainerCreating`：正在挂载存储卷、拉取镜像。
  * `ImagePullBackOff` / `ErrImagePull`：镜像拉取失败（如名称写错、没配置 Secret）。K8s 正在按指数退避策略重试（BackOff）。
  * `CrashLoopBackOff`：最著名的报错。容器启动了，但主进程立刻崩溃退出（非 0 状态码），kubelet 会重启它；又崩溃，又重启，最终陷入退避循环。
* **Running (运行中)**：容器的主进程正在运行，没有报错退出。
* **Terminated (已终止)**：容器运行结束。常见 Reason 包括：
  * `Completed`：正常退出（状态码 0）。
  * `OOMKilled`：超出了内存 Limit，被宿主机内核强制 Kill（状态码 137）。
  * `Error`：因其他原因异常退出（如状态码 1/255 等）。

### 3. 里程碑：Pod Conditions (Pod 状况) & 探针辨析
这是决定 Pod 是否真的“准备好接客”的关键。Pod 有一个 `Conditions` 数组，就像它的体检报告，包含：`PodScheduled`, `Initialized`, `ContainersReady`, `Ready`。

> **💡 深度辨析：Container State 和 Pod Conditions 的本质区别**
> 很多初学者分不清两者。一句话概括：
> * **Container State 反映的是“物理现实”**：进程到底死没死？
> * **Pod Condition 反映的是“业务逻辑”**：应用到底能不能用？
> 
> **经典场景**：一个 Java SpringBoot 容器，主进程已经启动了（此时 `Container State = Running`），但是由于 Spring 框架需要 30 秒来初始化依赖注入和连数据库，在这个期间，Readiness Probe 会一直失败，导致 `Pod Condition (Ready) = False`。
> 结论：虽然进程是活的 (Running)，但它还没准备好接客 (Not Ready)，所以 K8s 依然不会把流量放给它。
> 
> **实战输出对比**：
> 这个 Java 慢启动的例子，如果你敲命令，会看到这样违背直觉的画面：
> ```bash
> # 1. kubectl get pod 看到的是汇总信息
> $ kubectl get pod my-java-app
> NAME          READY   STATUS    RESTARTS   AGE
> my-java-app   0/1     Running   0          15s
> # 👆 发现了吗？STATUS 是 Running (阶段/状态)，但 READY 是 0/1 (Condition 未就绪)
> 
> # 2. kubectl describe pod 看到底层细节
...
> Containers:
>   my-java-container:
>     State:          Running <-- 物理进程是活的
>       Started:      Wed, 01 Jan 2026 10:00:00 +0800
>     Ready:          False
> ```

**这 4 项 Conditions 是怎么变成 True 的？这就引出了探针 (Probes)：**
* **Startup Probe (启动探针)**：专治“慢启动”应用。在它成功前，其他探针全部被挂起禁用。
* **Liveness Probe (存活探针)**：判断进程是否“假死”。如果失败，kubelet 会**杀掉容器并重启它**（从而可能引发 `CrashLoopBackOff`）。
* **Readiness Probe (就绪探针)**：判断应用是否“能接客”。如果失败，kubelet 会**把该 Pod 从 Service 的 Endpoints 列表中剔除**，从而改变 `Ready=False` 的 Condition，**但绝不会杀掉容器！**
